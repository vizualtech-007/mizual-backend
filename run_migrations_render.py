...
def run_migration_file(file_path, description):
    ...
    if not database_url:
        logger.info("❌ DATABASE_URL environment variable not found")
        return False
    ...
    try:
        ...
        logger.info(f"❌ Failed: {description}")
        logger.error(f"Error: {e.stderr}")
        return False
    except FileNotFoundError:
        logger.info("❌ psql command not found. PostgreSQL client not installed.")
        return False

def main():
    """Run all migrations in order"""
    logger.info("🚀 Starting Mizual Database Migrations on Render")
    logger.info("=" * 50)
    ...
    if not migrations_dir.exists():
        logger.info("❌ Migrations directory not found")
        sys.exit(1)
    ...
    for filename, description in migrations:
        ...
        if not file_path.exists():
            logger.info(f"❌ Migration file not found: {file_path}")
            sys.exit(1)
        
        success = run_migration_file(str(file_path), description)
        if not success:
            logger.info("❌ Migration failed. Stopping.")
            sys.exit(1)
        logger.info()
    
    logger.info("🎉 All migrations completed successfully!")
    logger.info("\nNext steps:")
    logger.info("1. Deploy your updated backend code")
    logger.info("2. Implement Feature 2 (Feedback System)")
    logger.info("3. Then Feature 3 (Progress Indicator)")
    logger.info("4. Finally Feature 1 (Follow-up Editing)")
